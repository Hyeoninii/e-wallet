...
- **정책 체인 관리**
  ...
  - 새 정책/역할 배포 시 자동으로 히스토리 배열에 추가되어 블록체인에 영구 저장
...
│   │   └── WalletContext.jsx
│   ├── pages/              # 페이지 컴포넌트
│   │   ├── CreateMultiSigPage.jsx
│   │   ├── MultiSigDashboardPage.jsx
│   │   ├── MultiSigTransactionsPage.jsx
│   │   ├── MultiSigMembersPage.jsx
│   │   ├── MultiSigSettingsPage.jsx
│   │   ├── RoleManagementPage.jsx
│   │   └── RuleDefinitionPage.jsx
│   └── utils/              # 유틸리티 함수
│       ├── multisig.js     # 다중 서명 지갑 관련 함수
│       ├── contractDeployer.js  # 컨트랙트 배포
│       ├── contractCompiler.js  # 컨트랙트 컴파일
│       └── contractGenerator.js # 컨트랙트 생성
├── package.json
└── README.md
```
...
OpenZeppelin의 `ReentrancyGuard`를 사용하여 재진입 공격 방지:
...
컨트랙트는 다양한 이벤트를 발생시켜 프론트엔드에서 상태 변화를 추적할 수 있습니다:

```solidity
event TransactionSubmitted(uint indexed txIndex, address indexed proposer, ...);
event TransactionConfirmed(uint indexed txIndex, address indexed owner);
event TransactionExecuted(uint indexed txIndex);
event ManagementTransactionSubmitted(...);
event PolicyUpdated(...);
event RoleCreated(...);
```
...
이슈와 풀 리퀘스트를 환영합니다. 프로젝트를 개선하기 위해 기여해주세요!

---

**주의**: 이 프로젝트는 교육 목적으로 제작되었습니다. 실제 자금을 다루는 경우 충분한 보안 감사를 수행하세요.
